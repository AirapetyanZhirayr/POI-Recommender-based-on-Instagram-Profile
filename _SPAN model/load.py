...
def process_traj(dname):  # start from 1
    '''
    :param L: number of POIs
    :param data (n_check-ins, [u, l, t]); [n-check-ins x 3]
    :param poi (L, [l, lat, lon]); [L x 3]
    :param
    :param
    :param
    :param
    ...
    for u_id in range(num_user+1):
        if u_id == 0:  # skip u_id == 0 (PADDING ID)
            continue
        ...
        user_mat1 = rst_mat1(user_traj[:-1], poi)  # (*M, *M, [dis, time])
